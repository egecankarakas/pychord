#!/usr/bin/bash
BOOTSTRAP_NODE="node325"
CHORD_NODES="node309 node310 node311 node312"
...
test_chord_removal_nodes() {
    ...
    for (( iter = 0; iter < max_itteration ; iter++)); do
      for ((i = 0 ; i < $max_nodes_todisable ; i++)); do
        echo "Starting New Max Node Loop"\
        logLocation="test/${folder_name}/nodeRemove_$max_nodes_todisable/iter_$iter/${removeLog}"
        CMD="mkdir test/${folder_name}/nodeRemove_$max_nodes_todisable/iter_$iter" 
        eval $CMD
        CMD="touch test/${folder_name}/nodeRemove_$max_nodes_todisable/${removeLog}"
        eval $CMD

        
        echo "Cord Nodes: $CHORD_NODES BOOTSTRAP_NODE: $BOOTSTRAP_NODE" >> "test/${folder_name}/nodeRemove_$max_nodes_todisable/iter_$iter/${removeLog}"
        start_bootstrap_node 
        start_chord_nodes  
        sleep 10 
        ...
        for i in $nodes_used;
        do
          echo "disabling: ${CHORD_NODES_arry[$nNumb]}"
          tm=$(date '+%T.%3N')
          CMD="./a2/scripts/chord.sh STOP_LOCAL_NODE" 
          echo "${CHORD_NODES_arry[$i]} has been stoped @ ${tm} " >> "test/${folder_name}/nodeRemove_$max_nodes_todisable/iter_$iter$/{removeLog}"
          echo "${CHORD_NODES_arry[$i]} has been stoped @ ${tm} "
          ssh -f "$node" $CMD 
          echo "Node $node disabled @" $tm
          sleep 15
        done
        tm=$(date '+%T.%3N')
        echo "Finished @ ${tm} " >> $nodeLogLocation
        sleep 10
        echo "stop bootstrap"
        stop_bootstrap_node
        echo "stop chord"
        ...
        CMD="mv node3* chord.log test/${folder_name}/nodeRemove_$max_nodes_todisable/iter_$iter"
        eval $CMD 
        sleep 10
      done
    done
