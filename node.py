...
class Node(BaseNode):
    ...
    async def stabilize(self): # -->
        if self.successor:
            ...
            if x and x.nid != self.nid and arc_contains(self.nid,self.successor.nid,x.nid):
                self.successor = x
    ...
    async def notify(self, bag): # <--
        ...
        log.debug(f"Node {bag['nid']} thinks that it might be predecessor of {self.nid}")
        if not self.predecessor or arc_contains(self.predecessor.nid,self.nid,bag['nid']):
            log.debug(bag)
            self.predecessor = RemoteNode.from_bag(bag) # TODO: Control it
            return op.OK_RESPONSE
        return op.FAIL_RESPONSE
