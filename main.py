...
def main():
    ...
    logging.basicConfig(filename='chord.log', level=logging.DEBUG, format='%(asctime)s.%(msecs)03d %(levelname)s:%(message)s')
    for handler in logging.getLogger().handlers:
        if not isinstance(handler, logging.StreamHandler):
            handler.setFormatter(logging.Formatter(
                '%(asctime)s.%(msecs)03d %(hostname)s: %(message)s', datefmt='%b %d %H:%M:%S'))
